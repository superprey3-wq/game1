...
const sourceUrlOverrides={
  "The Whisper Dance":"https://educacionsexual.org/en/sex-positions/the-whisper-dance-3/"
};

function verifiedUrl(title){
  return sourceUrlOverrides[title] || SOURCE_BASE+sourceSlug(title)+"/";
}

function makeVerifiedPool(category,groups){
  let n=0;
  return groups.flatMap(([baseTitle,variants])=>{
    const url=verifiedUrl(baseTitle);
    const fallback=category==="anal"?ANAL_CATALOG:STRAIGHT_CATALOG;
    const base={
      id:category+"-"+(n++),
      title:baseTitle,
      description:category==="oral"
        ?"Проверенная иллюстрированная страница для орального варианта пары мужчина + женщина. Нажмите ниже: сначала показана схема, затем пошаговое расположение партнёров."
        :category==="anal"
          ?"Проверенная позиция из иллюстрированного anal-каталога. Для вашей пары используйте вариант Penis–Anus / мужчина + женщина, указанный на странице."
          :"Проверенная иллюстрированная позиция для пары мужчина + женщина. На странице показана схема тел и пошагово объяснено расположение.",
      difficulty:"на странице источника",
      setting:"иллюстрированная инструкция",
      scene:category==="oral"?"oralLie":category==="anal"?"analSide":"face",
      note:"Адрес базовой страницы проверен перед добавлением. Варианты этой позиции находятся на той же странице.",
      source:{name:baseTitle,url,domain:"educacionsexual.org"},
      fallback
    };
    const variations=(variants||[]).map(v=>({
      ...
      id:category+"-"+(n++),
      title:baseTitle+" — "+v,
      description:"Реальная вариация «"+baseTitle+"». На открываемой странице есть иллюстрация базовой позы и отдельный раздел Variations с описанием этого варианта.",
      source:{name:baseTitle+" · "+v,url,domain:"educacionsexual.org"}
    }));
...
const pools={
  vaginal:makeVerifiedPool("vaginal",ordinaryGroups),
  oral:makeVerifiedPool("oral",oralGroups),
  anal:makeVerifiedPool("anal",analGroups)
};
...
function sourcePreview(pose,meta){
  return '<a class="catalog-preview" href="'+pose.source.url+'" target="_blank" rel="noopener noreferrer">'+
    ...
    '<span class="catalog-preview-kicker">Проверенная страница с иллюстрацией</span>'+
    '<strong>'+escapeHtml(pose.title)+'</strong>'+
    '<span class="catalog-preview-domain">Открыть рисунок и пошаговую инструкцию ↗</span>'+
  '</a>';
}
...
function render(category,pose){
  ...
  sourceFallback.textContent=category==="anal"?"Открыть весь иллюстрированный anal-каталог ↗":"Открыть весь каталог для пары мужчина + женщина ↗";
