...
class Pickup {
  ...
  collect(player) {
    if (this.kind === 'heart') { player.hearts++; game.addScore(100); AudioSys.sfxPickup(); }
      ...
      game.addScore(v); player.gems += 3 + player.perkRank('lapidary');
      spawnFloater(this.x, this.y - 4, '+' + v, '#c060e0'); AudioSys.sfxPickup();
    }
    else if (this.kind.startsWith('weap_')) game.giveItem(this.kind, this.x, this.y);
    else if (BUFFS[this.kind]) player.giveBuff(this.kind);
    else if (this.kind === 'orb') { player.heal(5); AudioSys.sfxOrb(); burst(this.x + 4, this.y, ['#50d8e8', '#f8f8ff'], 8, 1, -0.02); }
    else if (this.kind === 'key') { player.keys++; game.stats.items++; spawnFloater(this.x + 5, this.y - 6, 'GOLDEN KEY', '#ffe080'); AudioSys.sfxItem(); }
    else if (this.kind === 'chest') { game.addScore(1000); for (let i=0;i<3;i++) game.pickups.push(new Pickup(this.x-8+i*8,this.y-10-i*3,'heart')); AudioSys.sfxItem(); }
    else if (this.kind === 'elixir') { player.maxHp += 2; player.hp = player.maxHpTotal(); AudioSys.sfxOrb(); }
    else if (this.kind === 'relic') game.giveRelic(this.data || rollRelic(game.stage * 0.2));
    else if (this.kind === 'roast') { player.maxHp += player.perkRank('gourmand'); player.hp = player.maxHpTotal(); AudioSys.sfxOrb(); }
    else game.giveItem(this.kind, this.x, this.y);
    this.remove = true;
  }
  draw(g, camX, camY) {
    if (this.life < 90 && (this.life & 4)) return;
    const spr = SUBWEAPONS[this.kind] ? SUBWEAPONS[this.kind].icon() : this.kind === 'heart' ? Sprites.heart : this.kind === 'bigheart' ? Sprites.bigHeart : this.kind === 'gem' ? Sprites.gem : this.kind === 'orb' ? Sprites.orb : this.kind === 'roast' ? Sprites.roast : this.kind === 'chest' ? Sprites.chest : this.kind === 'key' ? Sprites.key : this.kind === 'scroll' ? Sprites.whipItem : this.kind === 'elixir' ? Sprites.elixir : this.kind === 'whip' ? Sprites.whipItem : this.kind.startsWith('weap_') ? weaponIcon(this.kind.slice(5)) : Sprites.soul;
    const bob=this.grounded?Math.round(Math.sin(this.t*.08)*(this.soul?2:1)):0;
    g.drawImage(spr,Math.floor(this.x-camX),Math.floor(this.y+bob-camY));
  }
}

class VictoryOrb {
  constructor(x,y){this.x=x;this.y=y;this.w=12;this.h=12;this.t=0;}
  update(){this.t++;if(this.t%6===0)spawnParticle(this.x+Math.random()*12,this.y+Math.random()*12,(Math.random()-.5)*.6,-.5,['#50d8e8','#f8f8ff','#ffe080'][(Math.random()*3)|0],24,-.01);}
  draw(g,camX,camY){const bob=Math.sin(this.t*.06)*3;g.drawImage(Sprites.orb,Math.floor(this.x-camX),Math.floor(this.y+bob-camY),12,12);}
}
